...
public class AllMatchesTab extends EntryListTab<AllMatchesTab.Entry> {
    ...
    protected void renderDecorations(MatrixStack matrices, int mouseX, int mouseY) {
        ...
        if (mouseY >= this.top && mouseY < this.bottom) {
            ...
            parent.renderTooltip(matrices, Text.of(String.join(", ", entry.statistic.ties())), mouseX, mouseY);
        }
    }
    ...
    public class Entry extends EntryListTab<Entry>.Entry {
        ...
        public void render(MatrixStack matrices, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
            ...
            draw(matrices, (this.index) + (statistic.plobby() ? "*" : ""), GAME_COLUMN, y, colour);
            draw(matrices, String.valueOf(statistic.placement()), PLACEMENT_COLUMN, y, colour);
            draw(matrices, ties(statistic.ties(), colour), TIES_COLUMN, y, colour);
            draw(matrices, statistic.deathCause(), DEATH_COLUMN, y, colour);
            draw(matrices, toTime(statistic.seconds()), TIME_COLUMN, y, colour);
            draw(matrices, String.valueOf(statistic.walls()), WALLS_COLUMN, y, colour);
            draw(matrices, statistic.map(), MAP_COLUMN, y, colour);
            draw(matrices, new SimpleDateFormat("dd/MM/yyyy").format(statistic.date()), DATE_COLUMN, y, colour);
        }
