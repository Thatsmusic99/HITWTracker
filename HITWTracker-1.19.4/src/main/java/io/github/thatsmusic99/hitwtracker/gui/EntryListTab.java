...
public class EntryListTab<T, C extends IColumn<T>> extends AbstractEntryListTab<EntryListTab<T, C>.Entry> implements Tab, LenientTab, IEntryListTab<T, C> {
    ...
    public void render(MatrixStack context, int mouseX, int mouseY, float delta) {
        ...
        drawTexture(context, this.left, this.top, (float)this.right, (float)(this.bottom + (int)this.getScrollAmount()), this.right - this.left, this.bottom - this.top, 32, 32);
        ...
        if (scrollLength > 0) {
            ...
            fill(context, this.left, startY, this.right, endY, -16777216);
            fill(context, start, startY, start + barLength, endY, -8355712);
            fill(context, start, startY, start + barLength - 1, endY - 1, -4144960);
        }
        // context.fillGradient(RenderLayer.getGuiOverlay(), this.left, this.top, this.right, this.top, -16777216, 0, 0);
    }

    @Override
    protected void renderDecorations(MatrixStack context, int mouseX, int mouseY) {
    ...
    public class Entry extends AlwaysSelectedEntryListWidget.Entry<Entry> implements IEntry<T> {
        ...
        protected void draw(MatrixStack context, String text, TextWidget column, int y, int colour) {
            this.draw(context, Text.of(text), column, y, colour);
        }

        protected void draw(MatrixStack context, Text text, TextWidget column, int y, int colour) {
            drawTextWithShadow(context, client.textRenderer, text, centre(text, column), y, colour);
        }
        ...
        public void render(MatrixStack context, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
            ...
            if (useFaces != null) {
                ...
                RenderSystem.setShaderTexture(1, HITWTrackerClient.get().getProfileManager().get(useFaces.apply(item)));
                drawTexture(context,
