...
public class Column<E, C extends IColumn<E>> extends TextWidget implements IColumn<E> {
    ...
    public void onHover(final @NotNull IEntry<E> entry, final @NotNull MatrixStack context, final int x, final int y) {
        if (this.onHover == null) return;
        final String result = this.onHover.apply(entry);
        if (result == null || result.isEmpty()) return;
        tab.parent.renderTooltip(context, Text.of(result), x, y);
    }
