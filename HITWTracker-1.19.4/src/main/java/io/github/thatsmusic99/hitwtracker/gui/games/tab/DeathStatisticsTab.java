...
public class DeathStatisticsTab extends EntryListTab<DeathStatisticsTab.Entry> {
    ...
    public class Entry extends EntryListTab<Entry>.Entry {
        ...
        public void render(MatrixStack matrices, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
            ...
            draw(matrices, stat.getReason(), DEATH_CAUSE_COLUMN, y, colour);
            draw(matrices, String.valueOf(stat.getCount()), COUNT_COLUMN, y, colour);
            draw(matrices, String.format("%.2f", stat.getCount() / (float) stat.getGames() * 100) + "%", RATE_COLUMN, y, colour);
        }
    }
}
