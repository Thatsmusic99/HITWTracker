...
public class MapStatisticsTab extends EntryListTab<MapStatisticsTab.Entry> {
    ...
    public class Entry extends EntryListTab<Entry>.Entry {
        ...
        public void render(MatrixStack matrices, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
            ...
            draw(matrices, stat.getMap(), MAP_COLUMN, y, colour);
            draw(matrices, String.valueOf(stat.getGames()), GAMES_COLUMN, y, colour);
            draw(matrices, String.format("%.1f", stat.getAvgPlacement()), AVERAGE_PLACEMENT_COLUMN, y, colour);
            draw(matrices, stat.getTopDeathCause(), TOP_DEATH_CAUSE_COLUMN, y, colour);
            draw(matrices, String.valueOf(stat.getTies()), TIES_COLUMN, y, colour);
            draw(matrices, String.valueOf(stat.getLargestTie()), LARGEST_TIE_COLUMN, y, colour);
            draw(matrices, stat.getMostTiedWith(), MOST_TIED_WITH_COLUMN, y, colour);
            draw(matrices, String.valueOf(stat.getWins()),WINS_COLUMN, y, colour);
            draw(matrices, String.valueOf(stat.getTopThrees()), TOP_THREES_COLUMN, y, colour);
            draw(matrices, String.valueOf(stat.getWalls()), WALLS_COLUMN, y, colour);
            draw(matrices, toTime(stat.getAverageTime()), AVERAGE_TIME_COLUMN, y, colour);
            draw(matrices, toTime(stat.getFastestTime()), FASTEST_TIME_COLUMN, y, colour);
        }
    }
}
