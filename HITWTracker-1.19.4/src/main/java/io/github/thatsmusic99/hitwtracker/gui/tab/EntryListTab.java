...
public abstract class EntryListTab<E extends AlwaysSelectedEntryListWidget.Entry<E>> extends AlwaysSelectedEntryListWidget<E> implements Tab, LenientTab {
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        if (scrollLength > 0) {
            ...
            fill(matrices, this.left, startY, this.right, endY, -16777216);
            fill(matrices, start, startY, start + barLength, endY, -8355712);
            fill(matrices, start, startY, start + barLength - 1, endY - 1, -4144960);
        }
    }

    public abstract class Entry extends AlwaysSelectedEntryListWidget.Entry<E> {

        protected void draw(MatrixStack matrices, String text, TextWidget column, int y, int colour) {
            this.draw(matrices, Text.of(text), column, y, colour);
        }

        protected void draw(MatrixStack matrices, Text text, TextWidget column, int y, int colour) {
            drawTextWithShadow(matrices, client.textRenderer, text, centre(text, column), y, colour);
        }
