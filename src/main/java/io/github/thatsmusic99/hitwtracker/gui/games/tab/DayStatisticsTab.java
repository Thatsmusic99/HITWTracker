...
public class DayStatisticsTab extends EntryListTab<DayStatisticsTab.Entry> {
    ...
    public class Entry extends EntryListTab<Entry>.Entry {
        ...
        public void render(DrawContext context, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
            ...
            draw(context, new SimpleDateFormat("dd/MM/yyyy").format(stat.date()), DATE_COLUMN, y, colour);
            draw(context, String.valueOf(stat.games()), GAMES_COLUMN, y, colour);
            draw(context, String.format("%.1f", stat.avgPlacement()), AVG_PLACE_COLUMN, y, colour);
            draw(context, String.valueOf(stat.tieCount()), TIE_COUNT_COLUMN, y, colour);
            draw(context, String.format("%.2f", stat.tieRate() * 100) + "%", TIE_RATE_COLUMN, y, colour);
            draw(context, String.valueOf(stat.winCount()), WIN_COUNT_COLUMN, y, colour);
            draw(context, String.format("%.2f", stat.winRate() * 100) + "%", WIN_RATE_COLUMN, y, colour);
            draw(context, String.valueOf(stat.topThreeCount()), TOP_THREE_COUNT_COLUMN, y, colour);
            draw(context, String.format("%.2f", stat.topThreeRate() * 100) + "%", TOP_THREE_RATE_COLUMN, y, colour);
