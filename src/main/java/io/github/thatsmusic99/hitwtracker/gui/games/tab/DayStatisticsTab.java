...
public class DayStatisticsTab extends EntryListTab<DayStatisticsTab.Entry> {
    ...
    public class Entry extends EntryListTab<Entry>.Entry {
        ...
        public void render(DrawContext context, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
            ...
            draw(context, String.format("%d (%.1f%%)", stat.tieCount(), stat.tieRate() * 100), TIE_COUNT_COLUMN, y, colour);
            draw(context, String.format("%d (%.1f%%)", stat.winCount(), stat.winRate() * 100), WIN_COUNT_COLUMN, y, colour);
            draw(context, String.format("%d (%.1f%%)", stat.topThreeCount(), stat.topThreeRate() * 100), TOP_THREE_COUNT_COLUMN, y, colour);
